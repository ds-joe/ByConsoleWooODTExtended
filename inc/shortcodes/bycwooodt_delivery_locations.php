...
add_shortcode('WooODT_Extended_delivery_points','function_WooODT_Extended_delivery_points');	
...
function function_WooODT_Extended_delivery_points( $atts, $content = null ){
	//echo "AAAA";
	$byc_delivery_locations=$get_byc_wooodt_data['byconsolewooodt_delivery_location'];

		//print_r($byc_delivery_locations);
?>
...
//$a= ksort($byc__locations[location]);	
...
foreach($byc_delivery_locations as $key => $value)
...
foreach($byc_delivery_locations as $key=>$val){
...

	{
		...
		array_push($new_ordered_array,$byc_delivery_locations[$key]);
...
$prepared_delivery_location_array=$new_ordered_array;
	...
	$prepared_delivery_location_array=$byc_delivery_locations;
		...
		foreach($prepared_delivery_location_array as $key => $value)
...
{	
	...
	if($index == "email_id_on_each_delivery_location")
...
if($atts["show_weekdays"]=='YES') {			
  ...
  <?php }
  				...
  				{ ?>
 					...
 					<td bgcolor="#FFFFFF" ><?php echo '<span style="color:red">CLOSED</span>'; ?> </td>
